...
- **Одна функция** — не нужно вызывать отдельно `configure_root`

## Установка

```bash
pip install git+https://github.com/sidorov-works/logger_utils@v0.4.0
```
...
1. **Первый вызов** `get_logger()` в процессе настраивает корневой логер:
   ...
   - Добавляет обработчики (файл + консоль или только консоль в Docker)
...
4. У логеров из shared модулей нет своих обработчиков, поэтому они прокидывают события корневому логеру
5. Корневой логер имеет обработчики → логи выводятся
